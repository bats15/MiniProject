...
class DiffusionUNet(nn.Module):
    ...
    def __init__(
        ...
        for i, mult in enumerate(channel_mult):
            ...
            for _ in range(num_res_blocks):
                self.encoder_blocks.append(
                    ResBlock(channels[-1], out_ch, time_dim, use_attention and i >= 2)
                )
        ...
        # Bottleneck
        self.bottleneck = ResBlock(channels[-1], channels[-1], time_dim, use_attention=True)
        ...
        for i, mult in enumerate(reversed(channel_mult)):
            ...
            for j in range(num_res_blocks + 1):
                ...
                self.decoder_blocks.append(
                    ResBlock(in_ch + (channels[-1] if channels else 0), out_ch, time_dim, 
                            use_attention and i <= 1)
                )
